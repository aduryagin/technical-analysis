...
it("sma update", () => {
  ...
  expect(sma.update({ time: 4, close: 5 })).toEqual({
    time: 4,
    value: 4,
    candle: { time: 4, close: 5 },
  });
  expect(sma.update({ time: 5, close: 5 })).toEqual({
    time: 5,
    value: 4,
    candle: { time: 5, close: 5 },
  });
  expect(sma.update({ time: 6, close: 5 })).toEqual({
    time: 6,
    value: 5,
    candle: { time: 6, close: 5 },
  });
});
