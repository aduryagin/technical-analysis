...
export function PMaxRSI({
  ...
  function calculate(candle, index) {
    ...
    const i2 = candle.close < candleStack[index - 1].close ? candleStack[index - 1].close - candle.close : 0
    ...
    const k1 = candle.high > candleStack[index - 1].close ? candle.high - candleStack[index - 1].close : 0
    const k2 = candle.high < candleStack[index - 1].close ? candleStack[index - 1].close - candle.high : 0
    const k3 = candle.low > candleStack[index - 1].close ? candle.low - candleStack[index - 1].close : 0
    const k4 = candle.low < candleStack[index - 1].close ? candleStack[index - 1].close - candle.low : 0
    ...
    const TR = Math.max(rsih - rsil, Math.abs(rsih - (result[result.length - 1]?.rsi || 0)), Math.abs(rsil - (result[result.length - 1]?.rsi || 0)))
